...
export interface Product {
  ...
  categories: { categoryId?: number; category?: { id: number; name: string }; id?: number }[];
  createdAt: string;
  updatedAt: string;
}
...
export const getFarmerProducts = (params?: { page?: number; limit?: number }) =>
  api.get<{ success: boolean; data: ProductsResponse }>("/products/my-products", { params });

export const createProduct = (payload: ProductPayload) =>
  api.post<{ success: boolean; data: Product; message: string }>("/products", payload);

export const updateProduct = (id: number, payload: ProductPayload) =>
  api.put<{ success: boolean; data: Product; message: string }>(`/products/${id}`, payload);

export const deleteProduct = (id: number) =>
  api.delete<{ success: boolean; message: string }>(`/products/${id}`);

export const uploadProductImages = (id: number, images: string[]) =>
  api.post<{ success: boolean; data: any; message: string }>(`/products/${id}/images`, { images });